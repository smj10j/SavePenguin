...
- (instancetype)init
{
	...
	self.defaultCachePolicy = ASIUseDefaultCachePolicy;
	self.accessLock = [[[NSRecursiveLock alloc] init] autorelease];
	return self;
}
	...
	if (!sharedCache) {
		@synchronized(self) {
			if (!sharedCache) {
				...
				sharedCache.storagePath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"ASIHTTPRequestCache"];
			}
		}
	}
	return sharedCache;
